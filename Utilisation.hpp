...
void JouerNotes(Instrument instrument, float rythme) 
{
...
void LirePartition(Instrument instrument,map<string, int>note_to_frequency, float rythme, string partition ) 
{
...
float ChoixRythme(vector<shared_ptr<Instrument>> instruments,int choixInstrument, string action)
{
	...
	return instruments[choixInstrument]->ChoisirRythme();
}

void ChoixLirePartition(Instrument instrument, vector<Instrument> instruments, int choixInstrument)
{
	int choixAction;
	cout << " Que voulez-vous faire avec cet instrument ?" << endl << "1. Jouer des notes" << endl << "2. Lire une partition" << endl << "3. Quitter" << endl << "> ";
	cin >> choixAction;
